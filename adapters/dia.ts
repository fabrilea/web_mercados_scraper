...
// lista (para detectar promo) y categoría. Catálogo online nacional único (no varía por
// sucursal) — el precio real por sucursal de Tandil requeriría otra fuente (ver CLAUDE.md).
...
// Categorías reales de nivel superior del sitio (api/catalog_system/pub/category/tree/2),
// filtradas a las de supermercado (se excluyen Indumentaria, Electro Hogar, Aire libre,
// Tecnología, Colchones y Sommiers, Hogar y Deco, Combos). "Frescos" agrupa Lácteos,
// Carnicería, Frutas y Verduras, Fiambrería, etc. — se resuelven por sub-categoría real
// vía categoriaDesdePathVtex en vez de quedar todos como un genérico "Frescos". Se puede
// sobreescribir con DIA_CATEGORIES (lista separada por comas de slugs de URL).
const CATEGORIAS_DEFAULT = ['almacen', 'bebidas', 'frescos', 'congelados', 'limpieza', 'perfumeria', 'desayuno', 'bebes-y-ninos', 'mascotas']
...
const PAGE_SIZE = 50
// El límite anterior (200) dejaba afuera la mayoría del catálogo (categorías de hasta
// ~1100 productos). La paginación igual corta sola cuando una página vuelve vacía o
// incompleta, así que este número actúa como techo de seguridad, no como objetivo.
const MAX_PRODUCTOS_POR_CATEGORIA = Number(process.env.DIA_MAX_PRODUCTOS || 10000)
...
const DiaAdapter: SupermercadoAdapter = {
  nombre: 'Supermercados DIA',
  fuente: 'scraping_web',
  async obtenerProductos(): Promise<ProductoPrecio[]> {
    const categoriasEnv = process.env.DIA_CATEGORIES
        ...
        try {
          ...
          for (const p of productos) {
            ...
            for (const item of items) {
              ...
              out.push({
                supermercadoId: 'Supermercados DIA',
                nombreOriginal,
          ...
          console.warn('DIA: error en categoría', categoria, 'desde', from, e)
          break
        }
      }
    }
